...
export default class Template {
  ...
  _uiHandleTheme() {
    ...
    document
      .querySelectorAll(
        '[data-toggle="theme"][data-theme="' + (themeEl ? themeEl.getAttribute("href") : "default") + '"]',
      )
      .forEach((link) => {
  ...
  _uiApiBlocks(mode = "init", block = false) {
    ...
    let blockAPI = {
      init: () => {
        ...
        document.querySelectorAll('[data-toggle="block-option"][data-action="content_toggle"]').forEach((btn) => {
          btn.innerHTML =
            ...
            iconBase +
            " " +
            (btn.closest(".block").classList.contains("block-mode-hidden") ? iconContentActive : iconContent) +
            '"></i>';
        });
      ...
      state_toggle: () => {
        ...
        if (
          elBlock.querySelector('[data-toggle="block-option"][data-action="state_toggle"][data-action-mode="demo"]')
        ) {
          setTimeout(() => {
